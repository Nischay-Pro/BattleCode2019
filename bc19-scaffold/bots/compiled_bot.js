// Transcrypt'ed from Python, 2019-01-14 09:44:22
...
var pilgrim_move = function (robot) {
	...
	if (robot.me.turn > constants$3.pilgrim_will_scavenge_closeby_mines_after_turns && robot.me.turn < constants$3.pilgrim_will_scavenge_closeby_mines_before_turns) {
	...
	if (robot.current_move_destination != null) {
		...
		else if (len (robot.mov_path_between_base_and_mine) == robot.mov_path_index + 1) {
			if (robot.mov_path_between_base_and_mine [robot.mov_path_index] == robot.current_move_destination) {
				var __left0__ = robot.mov_path_between_base_and_mine [robot.mov_path_index];
				var new_pos_x = __left0__ [0];
				var new_pos_y = __left0__ [1];
				return robot.move (new_pos_x - pos_x, new_pos_y - pos_y);
...
var pilgrim_full = function (robot) {
	...
	var passable_map = robot.get_passable_map ();
	var occupied_map = robot.get_visible_robot_map ();
	var directions$$1 = constants$3.directions;
	if (karb_map [pos_y] [pos_x] == 1 || fuel_map [pos_y] [pos_x] == 1) {
		var __left0__ = vision$1.sort_visible_friendlies_by_distance (robot);
		var unused_store = __left0__ [0];
		var friendly_units = __left0__ [1];
		if (friendly_units) {
			for (var f_unit of friendly_units) {
				...
				if (f_unit.unit == constants$3.unit_church || f_unit.unit == constants$3.unit_castle) {
...
var _make_church = function (robot) {
	...
	if (robot.karbonite > 50 && robot.fuel > 200) {
		robot.signal (0, 0);
		return robot.build_unit (constants$3.unit_church, max_resource_pos [1], max_resource_pos [0]);
	}
};
...
// Transcrypt'ed from Python, 2019-01-14 09:44:23
var constants$4 = {};
var utility$7 = {};
...
var _build_manager_church = function (robot) {
	var unit_type = constants$4.unit_prophet;
	var pos_x = robot.me.x;
	var pos_y = robot.me.y;
	var occupied_map = robot.get_visible_robot_map ();
	var passable_map = robot.get_passable_map ();
...
var default_production_order = function (robot) {
	var unit_type = robot.me.unit;
	if (unit_type == constants$4.unit_church) {
		return _build_manager_church (robot);
	}
	if (unit_type == constants$4.unit_castle) {
...
var find_chokepoints = function (robot, grid_radius) {
	...
	while (y < len (given_map)) {
		...
		while (x < len (given_map)) {
			...
			if (ratio < constants$6.chokepoint_modifier) {
...
var find_karbonite_rich = function (robot, grid_radius) {
	...
	while (y < len (given_map)) {
		...
		while (x < len (given_map)) {
			var ratio = get_map_ratio (x, y, given_map, grid_radius);
			if (ratio > constants$6.karbonite_modifier) {
				results.append (tuple ([x, y, ratio]));
			}
			x += sub_side;
		}
...
var find_fuel_rich = function (robot, grid_radius) {
	...
	while (y < len (given_map)) {
		...
		while (x < len (given_map)) {
			var ratio = get_map_ratio (x, y, given_map, grid_radius);
			if (ratio > constants$6.fuel_modifier) {
				results.append (tuple ([x, y, ratio]));
			}
			x += sub_side;
		}
...
var castle = function (robot) {
	...
	for (var f_unit of friendly_units) {
		if (f_unit.castle_talk == constants$7.unit_castle) ;
		else if (f_unit.castle_talk == constants$7.unit_church) ;
		else if (f_unit.castle_talk == constants$7.unit_crusader) ;
		else if (f_unit.castle_talk == constants$7.unit_pilgrim) {
			pilgrim_count++;
		}
		else if (f_unit.castle_talk == constants$7.unit_preacher) ;
		else if (f_unit.castle_talk == constants$7.unit_prophet) {
			prophet_count++;
		}
	}
	if (robot.step < 2 && robot.karbonite > 60) {
		...
		return castle_build (robot, constants$7.unit_pilgrim);
	}
	else if (robot.karbonite > 100 && robot.fuel > 200) {
		if (prophet_count < pilgrim_count) {
			return castle_build (robot, constants$7.unit_prophet);
		}
...
var _castle_initial_check = function (robot) {
	if (len (robot.fuel_mine_location_from_this_castle) == 0) {
		robot.fuel_mine_locations_from_this_castle = utility$9.get_relative_fuel_mine_positions (robot);
	...
	if (len (robot.karb_mine_location_from_this_castle) == 0) {
		robot.karb_mine_locations_from_this_castle = utility$9.get_relative_karbonite_mine_positions (robot);
		robot.karb_mine_occupancy_from_this_castle = (function () {
			var __accu0__ = [];
			for (var i = 0; i < len (robot.karbonite_mine_locations_from_this_castle); i++) {
				__accu0__.append (-(1));
			}
			return __accu0__;
		}) ();
	...
	if (robot.castle_health == null) {
		robot.castle_health = constants$7.castle_max_health;
	}
	...
	if (__mod__ (robot.pilgrim_build_number, 2) == 0) {
		for (var iter_i = 0; iter_i < len (robot.karb_mine_occupancy_from_this_castle); iter_i++) {
			if (robot.karb_mine_occupancy_from_this_castle [iter_i] == -(1)) {
				...
				return communications$1.encode_msg_without_direction (mine_pos [0], mine_pos [1]);
			}
		}
	}
...
var castle_build = function (robot, unit_type) {
	...
	var occupied_map = robot.get_visible_robot_map ();
	var passable_map = robot.get_passable_map ();
	var directions$$1 = utility$9.random_cells_around ();
	for (var direction of directions$$1) {
		if (!(utility$9.is_cell_occupied (occupied_map, pos_x + direction [1], pos_y + direction [0])) && passable_map [pos_y + direction [0]] [pos_x + direction [1]] == 1) {
			return robot.build_unit (unit_type, direction [1], direction [0]);
		}
	}
...
var MyRobot =  __class__ ('MyRobot', [BCAbstractRobot], {
	...
	get turn () {return __get__ (this, function (self) {
		...
		if (__t__ (__t__ (__eq__ (__mod__ (self.step, 200), 3)) && __eq__ (unit_type, constants$8.unit_castle))) {
			(function () {
				var __accu0__ = self;
