...
var _make_church = function (robot) {
	...
	for (var p_church_pos of directions$$1) {
		if (!(utility$5.is_cell_occupied (occupied_map, pos_x + p_church_pos [1], pos_y + p_church_pos [0])) && passable_map [pos_y + p_church_pos [0]] [pos_x + p_church_pos [1]] == 1 && karb_map [pos_y + p_church_pos [0]] [pos_x + p_church_pos [1]] != 1 && fuel_map [pos_y + p_church_pos [0]] [pos_x + p_church_pos [1]] != 1) {
			var count = 0;
			for (var direction of directions$$1) {
				if (!(utility$5.is_out_of_bounds (len (occupied_map), (pos_x + p_church_pos [1]) + direction [1], (pos_y + p_church_pos [0]) + direction [0]))) {
...
var _castle_initial_check = function (robot) {
	if (len (robot.fuel_mine_locations_from_this_castle) == 0) {
		var __left0__ = utility$9.get_relative_fuel_mine_positions (robot);
		var unused_store = __left0__ [0];
	...
	if (len (robot.karb_mine_locations_from_this_castle) == 0) {
		var __left0__ = utility$9.get_relative_karbonite_mine_positions (robot);
		var unused_store = __left0__ [0];
	...
	if (__mod__ (robot.pilgrim_build_number, 2) == 1) {
		for (var iter_i = 0; iter_i < len (robot.karb_mine_occupancy_from_this_castle); iter_i++) {
			if (robot.karb_mine_occupancy_from_this_castle [iter_i] == -(1)) {
				...
				var comms = communications$1.encode_msg_without_direction (mine_pos [0], mine_pos [1]);
				return comms;
			}
		}
	}
